...
/**
 * Format a decimal hours value as "Xh Ym" (e.g. 8.38 -> "8h 23m") instead of
 * a raw decimal, which is easy to misread as hours:minutes.
 * @param {number|null} decimalHours
 * @returns {string}
 */
    ...
    if (attendanceData) {
        ...
        if (isPresent && hoursValue !== null) {
            ...
            if (diff < 0) {
                ...
                lessOtCell = `<span class="hour-badge less"><i class="fas fa-arrow-down"></i> ${formatHoursMinutes(lessValue)}</span>`;
            } else if (diff > 0) {
                overtimeValue = diff;
                lessOtCell = `<span class="hour-badge overtime"><i class="fas fa-arrow-up"></i> ${formatHoursMinutes(overtimeValue)}</span>`;
            } else {
