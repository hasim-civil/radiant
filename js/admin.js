...
async function exportToCSV() {
    ...
    try {
        ...
        const dateList = getDaysInMonth(year, month).filter(d => d <= getTodayDate());
        ...
        for (const emp of employees) {
            ...
            for (const dateStr of dateList) {
                ...
                let lessOtText = '--';
                if (resolved.lessValue > 0) lessOtText = `-${resolved.lessValue.toFixed(2)}h`;
                else if (resolved.overtimeValue > 0) lessOtText = `+${resolved.overtimeValue.toFixed(2)}h`;
...
async function loadMonthlySummary() {
    ...
    try {
        ...
        const dateList = getDaysInMonth(year, month);
        const holidayMap = await getHolidaysInRange(firstDayStr, lastDayStr);
        ...
        document.getElementById('summaryAvgHours').textContent = `${avgHours.toFixed(1)}h`;
        document.getElementById('summaryLessHours').textContent = `${totalLess.toFixed(1)}h`;
        document.getElementById('summaryOvertime').textContent = `${totalOvertime.toFixed(1)}h`;
