    ...
    try {
        ...
        if (attendanceData) {
            ...
            if (attendanceData.totalHours !== null) {
                workingHours.textContent = `${attendanceData.totalHours.toFixed(2)}h`;
            }
...
async function loadMonthlyAttendance() {
    ...
    try {
        ...
        // Every day in the month up to today (future days aren't part of history yet)
        const dateList = getDaysInMonth(year, month).filter(d => d <= todayStr);
